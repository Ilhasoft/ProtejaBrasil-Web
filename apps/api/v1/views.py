...
from apps.api.v1.serializers import TypeProtectionNetworkSerializer, ProtectionNetworkSerializer, TypeViolationSerializer, FeedbackSerializer, ThemeSerializer
...
def typeprotectionnetwork(request):
    ...
    * **icon** - the URL IMAGE (string)
    ...
    ##Exemple:
        ...
        GET /api/v1/protection_network/types/
        HEADER {Authorization: token_authorization}
    ...
    if token and token.is_active:
        if request.method == "GET":
            ...
            serializer = TypeProtectionNetworkSerializer(types, many=True)
            return Response(serializer.data)
    else:
        data = {'return': '401_UNAUTHORIZED'}
        return Response(data=data, status=status.HTTP_401_UNAUTHORIZED)
...
def typeprotectionnetwork_getone(request, typeprotectionnetwork_id):
    ...
    * **icon** - the URL IMAGE (string)
    ...
    ##Exemple:
        ...
        GET /api/v1/protection_network/types/1/
        HEADER {Authorization: token_authorization}
    ...
    if token and token.is_active:
        if request.method == "GET":
            try:
                types = Type.objects.get(id=typeprotectionnetwork_id)
                serializer = TypeProtectionNetworkSerializer(types, many=False)
                return Response(serializer.data)
            except:
                data = {'return': '404_NOT_FOUND'}
                return Response(data=data, status=status.HTTP_404_NOT_FOUND)
...
def protectionnetwork(request):
    ...
    * **operatingdays** - the OPERATING DAYS (array)
    ...
    if token and token.is_active:
        if request.method == "GET":
            protectionnetworks = ProtectionNetwork.objects.all().order_by('name')
            serializer = ProtectionNetworkSerializer(protectionnetworks, many=True)
            return Response(serializer.data)
    else:
        data = {'return': '401_UNAUTHORIZED'}
        return Response(data=data, status=status.HTTP_401_UNAUTHORIZED)
...
def protectionnetwork_getone(request, protectionnetwork_id):
    ...
    * **operatingdays** - the OPERATING DAYS (array)
    ...
    ##Exemple:
        ...
        GET /api/v1/protection_network/1/
        HEADER {Authorization: token_authorization}
    ...
    if token and token.is_active:
        if request.method == "GET":
            try:
                protectionnetworks = ProtectionNetwork.objects.get(id=protectionnetwork_id)
                serializer = ProtectionNetworkSerializer(protectionnetworks, many=False)
                return Response(serializer.data)
            except:
                data = {'return': '404_NOT_FOUND'}
                return Response(data=data, status=status.HTTP_404_NOT_FOUND)
...
def protectionnetwork_getbyname(request, name):
    ...
    * **operatingdays** - the OPERATING DAYS (array)
    ...
    ##Exemple:
        ...
        GET /api/v1/protection_network/name/test/
        HEADER {Authorization: token_authorization}
    ...
    if token and token.is_active:
        if request.method == "GET":
            try:
                protectionnetworks = ProtectionNetwork.objects.filter(name__icontains=name).order_by(u'name')[:30]
                serializer = ProtectionNetworkSerializer(protectionnetworks, many=True)
                return Response(serializer.data)
            except:
                data = {'return': '404_NOT_FOUND'}
...
def protectionnetwork_getbynameandcoordenates(request, name, lat, long):
    ...
    * **operatingdays** - the OPERATING DAYS (array)
    ...
    ##Exemple:
        ...
        GET /api/v1/protection_network/name/test/position/-9.000,-36.0000/
        HEADER {Authorization: token_authorization}
    ...
    if token and token.is_active:
        if request.method == "GET":
            try:
                ...
                protectionnetworks = ProtectionNetwork.objects.filter(name__icontains=name).order_by(u'name')
                ...
                if len(protectionnetworks) > 0:
                    ...
                    serializer = ProtectionNetworkSerializer(filter, many=True)
...
def protectionnetwork_getforstate(request, initials):
    ...
    * **operatingdays** - the OPERATING DAYS (array)
    ...
    ##Exemple:
        ...
        GET /api/v1/protection_network/state/AL/
        HEADER {Authorization: token_authorization}
...
def protectionnetwork_getfortheme(request, theme_id):
    ...
    * **operatingdays** - the OPERATING DAYS (array)
    ...
    ##Exemple:
        ...
        GET /api/v1/protection_network/theme/1/
        HEADER {Authorization: token_authorization}
    ...
    if token and token.is_active:
        if request.method == "GET":
            try:
                ...
                serializer = ProtectionNetworkSerializer(filter_protectionnetworks, many=True)
                return Response(serializer.data)
            except:
                data = {'return': '404_NOT_FOUND'}
                return Response(data=data, status=status.HTTP_404_NOT_FOUND)
...
def protectionnetwork_getforstateandtheme(request, initials, theme_id):
    ...
    * **operatingdays** - the OPERATING DAYS (array)
    ...
    ##Exemple:
        ...
        GET /api/v1/protection_network/state/AL/theme/1/
        HEADER {Authorization: token_authorization}
...
def protectionnetwork_getfortype(request, type_id):
    ...
    * **operatingdays** - the OPERATING DAYS (array)
    ...
    ##Exemple:
        ...
        GET /api/v1/protection_network/type/1/
        HEADER {Authorization: token_authorization}
...
def protectionnetwork_getfortypeandcoordenates(request, type_id, lat, long):
    ...
    * **operatingdays** - the OPERATING DAYS (array)
    ...
    ##Exemple:
        ...
        GET /api/v1/protection_network/type/1/position/-39.0000,-75.0000/
        HEADER {Authorization: token_authorization}
        ...
        if request.method == "GET":
            try:
                ...
                if len(protectionnetworks) > 0:
                    ...
                    getone_protectionnetwork = ProtectionNetwork.objects.get(id=order_dict[0][0])
                    serializer = ProtectionNetworkSerializer(getone_protectionnetwork, many=False)
                    data = serializer.data
                else:
                    data = []
...
def protectionnetwork_getforthemeandcoordenates(request, theme_id, lat, long, distance):
    ...
    * **operatingdays** - the OPERATING DAYS (array)
    ...
    ##Exemple:
        ...
        GET /api/v1/protection_network/theme/1/position/-39.0000,-75.00000/radius/20000/
        HEADER {Authorization: token_authorization}
    ...
    if token and token.is_active:
        if request.method == "GET":
            try:
                ...
                serializer = ProtectionNetworkSerializer(new_filter, many=True)
                return Response(serializer.data)
            except Exception as e:
                data = {'return': e.args}
...
def typeviolation(request):
    ...
    * **action** - the ACTION (strint)
    ...
    if token and token.is_active:
        if request.method == "GET":
            typeviolation = TypeViolation.objects.all().order_by('name')
            serializer = TypeViolationSerializer(typeviolation, many=True)
            return Response(serializer.data)
    else:
        data = {'return': '401_UNAUTHORIZED'}
        return Response(data=data, status=status.HTTP_401_UNAUTHORIZED)
...
def typeviolation_getone(request, typeviolation_id):
    ...
    * **action** - the ACTION (strint)
    ...
    if token and token.is_active:
        if request.method == "GET":
            try:
                typeviolation = TypeViolation.objects.get(id=typeviolation_id)
                serializer = TypeViolationSerializer(typeviolation, many=False)
                return Response(serializer.data)
            except:
                data = {'return': '404_NOT_FOUND'}
                return Response(data=data, status=status.HTTP_404_NOT_FOUND)
...
def typeviolation_getfortheme(request, theme_id):
    ...
    * **action** - the ACTION (strint)
    ...
    ##Exemple:
        ...
        GET /api/v1/violation_type/theme/1/
        HEADER {Authorization: token_authorization}
    ...
    if token and token.is_active:
        if request.method == "GET":
            try:
                theme = Theme.objects.get(id=theme_id)
                typesviolation = TypeViolation.objects.filter(theme=theme).order_by(u'name')
                serializer = TypeViolationSerializer(typesviolation, many=True)
                return Response(serializer.data)
            except:
                data = {'return': '404_NOT_FOUND'}
                return Response(data=data, status=status.HTTP_404_NOT_FOUND)
...
def typeviolation_getfortype(request, typeprotectionnetwork_id):
    ...
    * **action** - the ACTION (strint)
    ...
    ##Exemple:
        ...
        GET /api/v1/violation_type/type/1/
        HEADER {Authorization: token_authorization}
    ...
    if token and token.is_active:
        if request.method == "GET":
            try:
                ...
                serializer = TypeViolationSerializer(filter_typesviolation, many=True)
                return Response(serializer.data)
            except:
                data = {'return': '404_NOT_FOUND'}
                return Response(data=data, status=status.HTTP_404_NOT_FOUND)
...
def typeviolation_getforcategory(request, category):
    ...
    * **action** - the ACTION (strint)
    ...
    ##Exemple:
        ...
        GET /api/v1/violation_type/category/Violation/
        HEADER {Authorization: token_authorization}
